...
# Ensure Alpine rootfs is writable for configuration
chmod -R 755 "$ALPINE_DIR/etc" 2>/dev/null || true
mkdir -p "$ALPINE_DIR/etc/apk" 2>/dev/null || true
: > "$ALPINE_DIR/etc/resolv.conf" 2>/dev/null || true
printf '%s\n' 'nameserver 8.8.8.8' >> "$ALPINE_DIR/etc/resolv.conf" 2>/dev/null || true
